...
envsubst '${APP_TITLE} ${AUTH_ISSUER} ${AUTH_CLIENT_ID} ${AUTH_REDIRECT_URI} ${AUTH_POST_LOGOUT_URI} ${AUTH_SCOPES} ${API_BASE_URL} ${WEBHOOK_TASKS} ${WEBHOOK_FOLDERS} ${WEBHOOK_FILES} ${WEBHOOK_SERVE_FILE} ${WEBHOOK_FILE_DELETE} ${WEBHOOK_UPLOAD_FORM} ${WEBHOOK_COLLECTORS} ${WEBHOOK_COLLECTOR_UPDATE} ${WEBHOOK_PDF_UPLOAD}' \
